...
import { StyleSheet, Pressable, Text, TouchableHighlight, View, TextInput } from 'react-native';
...
const AppInner = () => {
  const isLoggedIn = useSelector((state) => !!state.userSlice.email)
      ...
      {isLoggedIn ? (
        <Tab.Navigator>
          ...
          <Tab.Screen
            name="Schedules"
            component={Schedules}
            options={{headerShown: false, title: "약속"}}
          />
          <Tab.Screen
            name="Moments"
            component={Moments}
            options={{headerShown: false, title: "추억"}}
          />
        </Tab.Navigator>
      ) : (
